...
public class PrivateTemporaryMessageReceivedEvent extends PrivateMessageReceivedEvent<PrivateTemporaryMessageReceivedPacket> {
    private static final Set<String> TARGETS = EntrustEnvironment.operation(
            ...
            set -> {
                ...
                set.add("message-private-temporary");
            }
    );

    public PrivateTemporaryMessageReceivedEvent(ApricotProxy proxy, PrivateTemporaryMessageReceivedPacket packet) {
        super(
                proxy,
                packet
        );
    ...
    public void fireEvent(EventHandler<?> handler) {
        if (handler instanceof PrivateTemporaryMessageReceivedEventHandler eventHandler) {
            if (this.isExclusive()) {
