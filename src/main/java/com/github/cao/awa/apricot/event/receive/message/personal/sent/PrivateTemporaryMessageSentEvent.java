...
public class PrivateTemporaryMessageSentEvent extends PrivateMessageSentEvent<PrivateTemporaryMessageSentPacket> {
    private static final Set<String> TARGETS = EntrustEnvironment.operation(
            ...
            set -> {
                ...
                set.add("message-sent-private-temporary");
            }
    );

    public PrivateTemporaryMessageSentEvent(ApricotProxy proxy, PrivateTemporaryMessageSentPacket packet) {
        super(
                proxy,
                packet
        );
    ...
    public void fireEvent(EventHandler<?> handler) {
        if (handler instanceof PrivateTemporaryMessageSentEventHandler eventHandler) {
            if (this.isExclusive()) {
